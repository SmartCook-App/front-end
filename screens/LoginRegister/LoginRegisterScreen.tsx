    ...
    <View style={styles.container}>
      <BackgroundImage>
        ...
        <View style={styles.showViewRegisterLogIn}>
          <TouchableOpacity onPress={() => registerView()} style={styles.registerLoginButton}>
            {showRegister ? (
              <>
                <Text style={styles.registerLoginButtonText}>{LRL[lang]?.register}</Text>
                <View style={styles.line} ></View>
              </>
            ) : (
              <Text style={[styles.registerLoginButtonText, styles.buttonTextOpacity]}>{LRL[lang]?.register}</Text>
            )}
          </TouchableOpacity>
          <TouchableOpacity onPress={() => registerView()} style={styles.registerLoginButton}>
            {showRegister ? (
              <Text style={[styles.registerLoginButtonText, styles.buttonTextOpacity]}>{LRL[lang]?.logIn}</Text>
            ) : (
              <>
                <Text style={styles.registerLoginButtonText}>{LRL[lang]?.logIn}</Text>
                <View style={styles.line} ></View>
              </>
            )}
